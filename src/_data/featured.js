...
async function processRemoteImages(featuredproducts) {
...
module.exports = async function () {
  const featuredproductsCache = new AssetCache("airtable-featuredproducts");

  if (featuredproductsCache.isCacheValid(CACHE_DURATION)) {
    return featuredproductsCache.getCachedValue(); // This returns a promise
  }

  console.log("Cache expired. Fetching data from Airtable");

  let featuredproducts = await getProductsData();

  featuredproducts = await processRemoteImages(featuredproducts);

  await featuredproductsCache.save(featuredproducts, "json");
